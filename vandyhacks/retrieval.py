...
class Retrieval(object):
    ...
    def get_index(ext):
        ...
        for tr in soup:
            tds = tr.find_all('td')
            symbol = (tds[0].find_all('a'
                      )[1].text if len(tds[0].find_all('a'))
                      > 1 else '[No symbol available]')
        ...
        indices = sorted(indices.items(), key=lambda (k, v): (v, k),
                         reverse=True)
        weight_total = sum([pair[1] for pair in indices[:5]])
        indices = [(index[0], index[1] / weight_total) for index in
                   indices]
        return indices[:5]
    ...
    def hist_stock(symbol, start_date, end_date):
        ...
        stock = [(x[0], x[6]) for x in
                 fin.quotes_historical_yahoo_ochl(symbol, start_date,
                 end_date, asobject=True)]
        return stock
    ...
    def get_news(
        ...
        params = {
            ...
            'q': str(q),
            'begin_date': str(y) + ('0' + str(m))[-2:] + ('0'
                    + str(d))[-2:],
            'end_date': str(y) + ('0' + str(m))[-2:] + ('0'
                    + str(d))[-2:],
            }
        for param in params:
    ...
    def sentiment_analysis(headlines):
        ...
        for headline in headlines:
            ...
            tokens_without_stop_words = [t for t in tokens if t
                    not in stop_words]
        ...
        for i in range(len(headlines_without_stop_words)):
            sentences = \
                tokenize.sent_tokenize(headlines_without_stop_words[i])
        ...
        if len(headlines_without_stop_words) != 0:
            return total_compoud_value_for_day \
                / len(headlines_without_stop_words)
        else:
            return 0
